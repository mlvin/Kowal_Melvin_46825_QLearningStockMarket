...
class CustomTradingEnv(StocksEnv):
    ...
    def _old_observation(self):
        """
        Wandelt die observation des environments in eine benutzerdefinierte Darstellung mit zusätzlichen Rückggabeparametern
        um, damit präzisere Vorhersagen erstellt werden können. -> Die observation des environments gibt lediglich
        den aktuellen Kurs sowie Veränderung zum Vortag an.

            Args:
                observation: observation des environemnts.

            Returns:
                tuple: Ein Tupel, dass die benutzerdefinierte observation darstellt.
                -> Indikatoren für Trend, Momentum & RSI als Tupel
        """
        ...
        # Alle Schlusskurse der vergangen Tage laden -> definiert durch den unteren Frame-Bound
        prices = [obs[0] for obs in observation]

        # Berechnung des Smooth-Moving-Averages (SMA) -> Gleitender Preisdurchschnitt der letzten X-Tage
        # Wenn Durchschnitt positiv: 1 -> Buy, sonst 0 -> Sell
        sma = np.mean(prices)
        sma_indicator = 1 if observation[-1][0] > sma else 0  # Trend

        # Berechnung des Relative-Strength-Indexes (RSI) um die "Marktstärke" zu messen -> Erkennen ob der Markt
        # überkauft oder überverkauft ist.

        # Preisänderungen der letzten X-Tage

        price_changes = [obs[1] for obs in observation]

        # Berechnung der durchschnittlichen positiven und negativen Preisänderungen
        ...
        # Indikator: Falls Wert des RSI < 30 -> Überkaufter Markt: -1 -> Sell
        # Falls Wert > 70 -> Überverkaufter Markt: 1 -> Buy
        # Sonst neutral: 0

        rsi_indicator = 1 if rsi < 30 else (-1 if rsi > 70 else 0)

        # Berechnung des Momentums -> Ist der Preis höher oder niedriger als zum ersten Tag innerhalb der observation?
        # Falls höher: 1 -> Buy, sonst 0 -> Sell
        momentum = 1 if (observation[-1][0] - observation[0][0]) > 0 else 0

        # Rückgabe der Indikatoren als Tupel

        return (sma_indicator, rsi_indicator, momentum)
